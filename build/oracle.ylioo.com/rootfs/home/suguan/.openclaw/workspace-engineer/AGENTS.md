...
- Do not assume attachments were auto-ingested. When a prompt includes `[media attached: /absolute/path (mime) | name]`, use the exact path as `filePath` and the visible `name` as `fileName` if you choose to ingest the file.
